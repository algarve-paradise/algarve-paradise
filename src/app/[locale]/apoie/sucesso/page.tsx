...
export const metadata: Metadata = {
  title: "Donativo Confirmado · Apoie",
  description: "O seu donativo foi recebido com sucesso. Obrigado pelo apoio ao Algarve TV Paradise.",
};
...
export default async function SuccessPage({ searchParams }: Props) {
  ...
  const metodo = params.metodo ?? "stripe";
  const methodLabel = METHOD_LABELS[metodo] ?? metodo;
  const txRef = generateRef(params.valor ?? "0", metodo);
          ...
          <div className="w-full max-w-lg space-y-8 text-center">
            ...
            <div className="space-y-3">
              ...
              <p className="text-[15px] leading-7 text-muted-foreground">
                O seu contributo ajuda a manter o Algarve TV Paradise ativo,
                gratuito e independente para toda a comunidade.
              </p>
            </div>
                ...
                <div className="p-7">
                  <div className="grid gap-4 sm:grid-cols-2">
                    <div className="rounded-[1rem] bg-[var(--dt-color-bg)] px-4 py-3 text-left">
                      <p className="text-[11px] uppercase tracking-[0.2em] text-muted-foreground">Valor doado</p>
                      <p className="mt-1 font-heading text-3xl text-foreground">
                        €{valor.toFixed(2)}
                      </p>
                    </div>
                    <div className="rounded-[1rem] bg-[var(--dt-color-bg)] px-4 py-3 text-left">
                      <p className="text-[11px] uppercase tracking-[0.2em] text-muted-foreground">Método</p>
                    ...
                    <div className="col-span-full rounded-[1rem] bg-[var(--dt-color-bg)] px-4 py-3 text-left">
                      <p className="text-[11px] uppercase tracking-[0.2em] text-muted-foreground">Referência da transação</p>
                  ...
                  <div className="mt-5 flex items-center gap-2 rounded-[1rem] border border-amber-200 bg-amber-50 px-4 py-3">
                    <p className="text-[12px] leading-5 text-amber-700">
                      <span className="font-semibold">Modo demo:</span> Nenhum valor foi cobrado. Este recibo é apenas ilustrativo.
                    </p>
                  </div>
            ...
            <p className="text-sm text-muted-foreground">
              Partilhe com quem gosta do Algarve e ajude-nos a crescer.{" "}
              <Link
